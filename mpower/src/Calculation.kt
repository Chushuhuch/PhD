...
//    proveInequality( Ai1r, WR( 0.0, 0.0 ), WR( 6.0, 0.1 ), Direction.Max, Number( 0.0 ) )
    ...
    val Ai = (
            ExpressionNode( "4 (w / ln - 1)" ) { wr: WR -> 4.0 * ( wr.w.vOverLog1p() - 1.0 ) } * ROverRPWExpression() +
            ExpressionNode( "4" ) { wr: WR -> Number( 4.0 ) } * ( WOverRPWExpression() - ROverRPWExpression() ) -
                    ( ExpressionNode( "w + 3" ) { wr: WR -> wr.w + 3.0 } * WOverRPWExpression() -
                            ExpressionNode( "1 - w" ) { wr: WR -> 1.0 - wr.w } * ROverRPWExpression()
...
//                            ExpressionNode( "ln" ) { wr: WR -> wr.w.log1p() }
//            ) / ExpressionNode( "2 (1 + r)^2 (r + w)^2" ) { wr: WR -> 2.0 * ( ( 1.0 + wr.r ) * ( wr.r + wr.w ) ).pow( 2.0 ) }
