...
We use the notation $f_\pm = \max( \pm f, 0 )$.
...
see, e.g., \cite[Theorem~1.4.1]{Shar}.
Denote by $u_n$ the primitive of $v_n$ such that $u_n(-1)=0$.

{\color{red}
It is easy to see that $u_n \rightrightarrows u$, and thus $\eps_n := -\inf u_n \to 0$.
Define $\delta_n$ by the relation
\begin{equation}
\label{left_delta}
...
$$
We claim that $\|\tilde v_n - v_n\|_{L^{p(x)}(-1, 1)} \to 0$.
Indeed, by (\ref{left_delta}) the measure of the set
$$
{\cal A}_n=\{ x \in [-1, -1 + \delta_n] \,:\, (v_n)_- \ge \sqrt{\eps_n} \}
$$
tends to $0$ as $n \to \infty$.
Since $v_n \to u'$ in $L^{p(x)}$, we have
$$
\|(v_n)_-\|_{L^{p(x)}({\cal A}_n)} \le \|u'\|_{L^{p(x)}({\cal A}_n)} + \|v_n - u'\|_{L^{p(x)}({\cal A}_n)} \to 0.
$$
Since evidently
$$
\|(v_n)_-\|_{L^{p(x)}([-1, -1 + \delta_n] \setminus {\cal A}_n)} \to 0,
$$
we have
$$
...
Denote by $\tilde u_n$ the primitive of $\tilde v_n$ such that $\tilde u_n(-1) = 0$.
By construction, $\tilde u_n \ge 0$.
Set $\tilde\eps_n = \tilde u_n(1) \to 0$, define $\tilde\delta_n$ by the relation
$$
...
$$
By the same reason as above, $\|\hat v_n - \tilde v_n\|_{L^{p(x)}(-1, 1)} \to 0$.

Denote by $\hat u_n$ the primitive of $\hat v_n$ such that $\hat u_n(-1) = 0$.
By construction, $\hat u_n \ge 0$, $\hat u_n(1) = 0$, and $\hat u_n' \to u'$ in $L^{p(x)}(-1, 1)$.
}

By embedding $L^{p(x)}[-1, 1]\mapsto L^1(-1, 1)$ we have $\hat u_n \to u$ in $\Wf(-1, 1)$.
Further, since $| \sqrt{ 1 + x^2 } - \sqrt{ 1 + y^2 } | \le | x - y |$ for all $x$ and $y$,
the relation $\hat u_n' \to u'$ in $L^{p(x)}$ implies $\I( \hat u_n ) \to \I( u )$.

By \cite[Theorem 1]{Br}, the convergence $\hat u_n \to u$ in $\Wf(-1, 1)$ implies weak convergence $\hat u_n^* \rightharpoondown u^*$ in $\Wf(-1, 1)$.
By the Tonelli theorem, the functional $\I$ is sequentially weakly lower semicontinuous (see, e.g., \cite[Theorem 3.5]{BGH}).
Thus,
$$
\I(u^*) \le \liminf\limits_n \I(\hat u_n^*) \le \lim\limits_n \I(\hat u_n) = \I(u).
$$
\end{proof}
...
function $B_\infty(w)$ estimating $\partial^2_{ww} B(w, \infty)$ from above. This
...
and to an anonymous referee for important advices.
