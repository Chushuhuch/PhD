...
We construct the function with support on a cylinder with the segment $[x_1, x_2]$ as an axis, and with nonzero derivatives only in circumferences of the border of the cylinder.
Namely,
$$
...
It is easy to see that $u^*$ has similar shape with $y_1, y_2$ becoming ${y_1 - y_2 \over 2}, {y_2 - y_1 \over 2}$, and both $s$ and $t$ changed to ${s + t \over 2}$.

To evaluate $I(u)$ and $I(u^*)$ we calculate areas $A_s$ with derivative ${1 \over s}$ (for ${1 \over t}$ and ${2 \over s + t}$ it is calculated the same way)
and $A_\delta$ with derivative $\delta$:
\begin{multline*}
\begin{split}
& A_s & = & \delta s \int\limits_{w - \varkappa}^w |B^{n - 1}_r| dr = C \delta s ( w^n - ( w - \varkappa )^n ) \\
& A_\delta & = & \int\limits_{w - \varkappa}^w |S^{n - 1}_r| ( y_2 - y_1 - (w - r) \delta (s + t) ) dv
\end{split}
\\
= C (y_2 - y_1) ( w^{n - 1} - ( w - \varkappa )^{n - 1}
- C \delta (s + t) ( { w^n - w (w - \varkappa)^{n - 1} \over n - 1 } - { w^n - ( w - \varkappa )^n \over n } )
\\
= C (y_2 - y_1) ( w^{n - 1} - ( w - \varkappa )^{n - 1}
- C \delta (s + t) ( w^n - (w - \varkappa)^{n - 1} ( w + \varkappa (n - 1) ) )
\end{multline*}
for some absolute constants $C$.

Take $\varkappa = {w \over 2}$, ${1 \over s} = {1 \over t} = w^2$, $\delta = w^4$ and set $w \to 0$.
Then
\begin{eqnarray*}
& A_s & = C w^{n + 2} \\
...
\\ = ( w^4 ( {p(\bar{x}_1) \over 2} + o(1) ) + w^4 ( {p(\bar{x}_2) \over 2} + o(1) ) C w^{n + 2}
+ ( w^8 {P \over 2} + o(1) ) C w^{n - 1} ( 1 + o(1) )
\\ -( w^4 ( {p(\hat{x}_1) \over 2} + o(1) ) + w^4 ( {p(\hat{x}_2) \over 2} + o(1) ) C w^{n + 2}
- ( w^8 {1 \over 2} + o(1) ) C w^{n - 1} ( 1 + o(1) )
...
Now take arbitrary $s$ and $t$, $\varkappa = {w \over 2}$, $\delta = w^2$ and set $w \to 0$.
Then
\begin{eqnarray*}
& A_s & = C s w^{n + 2} \\
& A_\delta & = C w^{n - 1} ( 1 + o(1) )
