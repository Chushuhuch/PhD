...
\begin{thm}
Suppose $I(u^*) \le I(u)$ for any $u \in \Wf$.
...
We construct a function $u \in \Wf$ with nonzero gradient only in circumferences of $x_1, x_2$ and around the border of a cylinder with axis $[x_1, x_2]$.
Namely,
$$
u(x) = \min\Big(
...
\Big)
$$
for some $c_1, c_2 > 0$ as inverse base derivatives along $y$,
$\B_1', \B_2' \in \Real^{n - 1}$ as base derivatives along $x'$,
$\delta > 0$ as side gradient magnitude,
$w > 0$ as cylinder width,
and $h > 0$ as highest function value.
...
The left base of $S = \supp u$ is the set of solutions to equation system
$$
{y - y_1 \over c_1} + (x' - x_0') \cdot \B_1' = 0; \qquad |x' - x_0'| \le w,
$$
which is an $n-1$-dimensional ball of radius $w$ stretched in one direction to measure $\sqrt{w^2 + c_1^2 w^2 |\B_1'|^2}$ instead of $w$ in radius.
