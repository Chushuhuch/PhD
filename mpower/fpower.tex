...
To find the actual maximum we take
$A_\infty(w) = A(w, \infty) = 2 - {1 \over 2} ( \ln(w + 1) + 3 {\ln(w + 1) \over w} ).$
We prove the maximum is unique by ensuring that $A_\infty$ is convex upwards for $w \in [1, 4]$.
Further, we use standard computational methods to find the extremum.
Estimating $A_\infty''(w)$ results in
\begin{center}
\begin{tabular} {|c|c|c|c|}
\hline Region & & Mesh step & Inequality
\\
\hline $[1, 4]$ & & $3 \cdot 10^{-3}$ & $A_\infty''|_{[1, 4]} \le -0.13$ \\
\hline\end{tabular}
\end{center}
The maximum is achieved for $w \approx 1.816960565240$ and equals approximately to $0.627178211634$.

To prove the inequality (\ref{calcHalf}) we split $(w, q) \in \Real_+ \times [0, 1.36]$
into four regions, see Fig.~2:
\begin{eqnarray*}
 &&R_6 = [0, 3] \times [0, 1.36],\quad R_7 =[5, \infty] \times [0,1.36], \\
 && R_8 = [3, 5] \times [0, 1.3],\quad R_9 = [3,5] \times [1.3, 1.36].
\end{eqnarray*}
...
\put(90,43){\LARGE $R_6$}
\put(315,43){\LARGE $R_7$}
\put(201,40){\LARGE $R_8$}
\put(201,85){\LARGE $R_9$}
...
On each of these regions we prove the inequality in the same was as for inequality (\ref{calc}),
...
\hline $R_7$ & & $2 \cdot 10^{-3}$ & $1.36 \cdot 10^{-2}$  & $A_4 \le 0.4992$ \\
\hline $R_8$ & & $2 \cdot 10^{-3}$ & $1.3 \cdot 10^{-3}$  & $A_1 \le 0.498$ \\
\hline $R_9$ & & $2 \cdot 10^{-4}$ & $6 \cdot 10^{-6}$  & $A_1 \le 0.49996$ \\
\hline\end{tabular}
\end{center}
