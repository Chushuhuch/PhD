...
\begin{thm}
Suppose that $\widehat\I(u^*) \le \widehat\I(u)$ for any nonnegative function $u \in \Wf(\Omega)$.
...
We set $\varkappa\equiv {h \over \delta} := {w \over 2}$ ($\varkappa$ is the width o    f the lateral layer with non-zero gradient).
...
Similarly, $\nabla w = (\B_2', -\frac{1}{c_2})$ on the set $A_2$ with $|A_2| = C_1 \delta c_2 w^n$.
...
appropriate piecewise constant function $A_2(w,r)$ estimating
$A(w,{1 \over r})$ from above.

In a similar way, for $(w,q) \in R_4$ we put $v = {1 \over w}$ and
construct appropriate piecewise constant function $A_4(v,q)$
...
$$
A(w, \infty) = 2 - {1 \over 2} ( \ln(w + 1) + 3 {\ln(w + 1) \over
w} )
$$
and claim that that $A(w, \infty)$ is concave for $w \in [1, 4]$.
To prove this we construct appropriate piecewise constant
function $A_\infty(w)$ estimating $A_\infty''(w)$ from above. This
...
\\
\hline $1\le w\le 4$ & & $3 \cdot 10^{-3}$ & $A_\infty \le -0.13$ \\
\hline\end{tabular}
\end{center}
...
A(w,\infty)\approx 0.627178211634$, and (\ref{calc}) follows.

To prove inequality (\ref{calcHalf}) we split $(w, q) \in \Real_+
\times [0, 1.36]$ into four regions, see Fig.~2:
\begin{eqnarray*}
...
\put(390,5){\large $w$} \put(30,5){$0$} \put(165,5){$3$} \put(255,5){$5$}

\put(19,20){$0$} \put(11,70){$1.3$} \put(5,80){$1.36$} \put(10,110){\large $q$}

\put(30,20){\vector(1,0){370}}
\put(30,80){\line(1,0){360}} \put(165,75){\line(1,0){90}}

\put(30,20){\vector(0,1){90}}
\put(165,20){\line(0,1){60}} \put(255,20){\line(0,1){60}}

\put(90,43){\LARGE $R_6$} \put(315,43){\LARGE $R_9$}
...
In these regions we used piecewise constant functions $A_1$ and
$A_4$ introduced earlier. These functions were calculated on
...
Inequality
\\
\hline $R_6$ & & $3 \cdot 10^{-3}$ & $1.36 \cdot 10^{-3}$ & $A_1 \le 0.498$ \\
\hline $R_7$ & & $2 \cdot 10^{-3}$ & $1.3 \cdot 10^{-3}$  & $A_1 \le 0.498$ \\
\hline $R_8$ & & $2 \cdot 10^{-4}$ & $6 \cdot 10^{-6}$  & $A_1 \le 0.49996$ \\
\hline $R_9$ & & $2 \cdot 10^{-3}$ & $1.36 \cdot 10^{-2}$  & $A_4 \le 0.4992$ \\
\hline\end{tabular}
\end{center}
...
Authors are grateful to M.~Surnachev for simplification of the proof of Theorem \ref{mainThm}.
