...
$K_{ss}>0$, and $K_{xx}\ge0$ since $p$ is convex. Thus, the convexity of $K$ is equivalent to $\det(K'')\ge0$ in the sense of distributions. Direct calculation gives
...
%\begin{eqnarray*}
%K_s & = & (1 + w)^\frac{q + 1}{2} + s \frac{q + 1}{2} (1 + w)^\frac{q - 1}{2} w' = (1 + w)^\frac{q - 1}{2} (1 - w q) \\
%K_x & = & s (1 + w)^\frac{q + 1}{2} \frac{q'}{2} \Ln.
%\end{eqnarray*}
%Further,
%\begin{align*}
%K_{ss} &= (1 + w)^\frac{q - 3}{2} \frac{q - 1}{2} w' (1 - w q) - (1 + w)^\frac{q - 1}{2} w' q
%= \frac{w}{s} (1 + w)^\frac{q - 3}{2} (w q + 1) (q + 1)
%\\ K_{sx} &= (1 + w)^\frac{q + 1}{2} \frac{q'}{2} \Ln,
...
%\\ K_{xx} &= s ( (1 + w)^\frac{q + 1}{2} ( \Ln \frac{q'}{2} )^2 + (1 + w)^\frac{q + 1}{2} \frac{q''}{2} \Ln )
...
\begin{equation}
\label{qq''}
q q'' \ge q'^2 {\cal A}(q)\equiv q'^2\cdot\sup\limits_{w>0} A(q, w),
...
$$
\det(\K''(c, d, y))=\frac 1{(1+d^2)^{\frac 32}}\cdot\big[(K_{yy}K_{ss}-K_{sy}^2)(K-K_s s)-K_{ss}K_y^2d^2\big]
$$
(here $s=\frac c{\sqrt{1+d^2}}$). Therefore, if $K_y\not\equiv0$ we can choose $d$ sufficiently large so that  $\det(\K''(c, d, y))<0$, a contradiction.
\end{proof}


\section{Appendix}

To prove inequality (\ref{calc}) we divide all the values $(q, w) \in \Real_+ \times \Real_+$ into the following five regions.
$R_1 = [0, 1] \times [0, 6]$,
$R_2 = [1, \infty] \times [0, 1]$,
$R_3 = [1, \infty] \times [1, 6]$,
$R_4 = [0, 1] \times [6, \infty]$,
$R_5 = [1, \infty] \times [6, \infty]$.
On each of these regions we numerically prove the inequality.
...
For $(q, w) \in R_1$ we have
\begin{multline*}
A(q, w) = \frac{
q ( 4 w - ( w + 3 ) \ln( w + 1 ) ) - (1 - {1 \over w}) \ln( w + 1 ) + 4 {w \over \ln( w + 1 )} - 4
}{
2 ( q w + 1 )
} \cdot {q \over q + 1}
\\ = \frac{
[4 q w] - [q (w + 3) \ln(w + 1)] - [\ln(w + 1)] + [{\ln(w + 1) \over w}] + [4 {w \over \ln(w + 1)}] - [4]
}{
[2 (q w + 1)]
} \cdot [{q \over q + 1}].
\end{multline*}
Since every function in brackets is monotone on $R_1$ in both variables,
we can estimate value of $A$ by estimating values of each of the monotone functions.
Also, we can present $R_1$ as a union of subregions $S_i$,
and estimate $A$ separately on each of them as follows.

\begin{multline*}
\max\limits_{A_1} A \le
\max\limits_i \Big\{
\Big(
    \max\limits_{S_i} [4 q w]
    - \min\limits_{S_i} [q (w + 3) \ln(w + 1)]
    - \min\limits_{S_i} [\ln(w + 1)] \\
    + \max\limits_{S_i} [\textstyle{\ln(w + 1) \over w}]
    + \max\limits_{S_i} [4 \textstyle{w \over \ln(w + 1)}]
    - [4]
\Big)
/ \min\limits_{S_i} [2 (q w + 1)]
\cdot \max\limits_{S_i} [{q \over q + 1}]
\Big\}
\end{multline*}
If $S_i$ is a rectangle,
an optimum of a monotone function can be estimated through values in the corners of $S_i$.

Thus, for every region $R_1, R_2, R_4, R_5$ we take function $A$
with inverted variables as described above,
and extract monotone parts in the formula.
Further, we invert corresponding components of a region
and partition it in $10^k$ parts in each variable obtaining $10^{2k}$ rectangular subregions.
We take $k$ large enough to achieve inequality $\sup\limits_{R_j} A < 0.62$ using described technique.

For each of the regions we provide monotone terms extraction and partition fineness parameter $10^k$.

For $R_2$ we take $r = {1 \over q}$ and
\begin{multline*}
A(\textstyle{1 \over r}, w) =: A^q(r, w)
= \Big(
    [4 ({w \over \ln(w + 1)} - 1) r]
    + [3 (w - \ln(w + 1)]
    - [w \ln(w + 1)] \\
...
/ [2 (1 + r)].
\end{multline*}
To avoid division by zero we evaluate it as $0.5$ at origin.

For $R_4$ we take $v = {1 \over w}$ and
\begin{multline*}
A(q, {\textstyle{1 \over v}}) =: A^w(q, v)
= \Big(
    [ 4 q ]
    - [ q ( 1 + 3v ) \ln(1 + {\textstyle{1 \over v}}) ]
    - [ v \ln(1 + {\textstyle{1 \over v}}) ]
    + [ v ] \cdot [ v \ln(1 + {\textstyle{1 \over v}}) ] \\
    + [ \textstyle\frac{4}{\ln(1 + {\textstyle{1 \over v}})} ]
    - [ 4 v ]
\Big)
* \frac{[ q ]}{[ v + q ]}
/ [ 2 (q + 1) ].
\end{multline*}
To avoid divisioin by zero we estimate the function as zero
since senior term of the nominator $-q ( 1 + 3v ) \ln(1 + {\textstyle{1 \over v}})$ is negative.

For $R_5$ we take
$$
A(\textstyle{1 \over r}, {\textstyle{1 \over v}}) =: A^{qw}(r, v)
= \frac{
    \big[ 4 {r \over \ln(1 + {\textstyle{1 \over v}})} \big]
    + [ 4 (1 - r v) ]
    - [ r (v - v^2) + 3 v + 1) \ln(1 + {\textstyle{1 \over v}}) ]
}{
[ 2 (1 + r) (1 + r v) ]
}.
$$

Finally, on $R_3$ we numerically prove that $A^q_r < 0$ using
\begin{multline*}
A^q_r(r, w) =
\Big(
    [ 4 \textstyle{w \over \ln(w + 1)} ]
    \cdot [ w - r^2 ]
    + [4] * ( [ r^2 ] - [ 2 r w + w^2 + 2 w ] ) \\
    + ( [ r^2 + 2 r (3 + w) + 4 + 3 w + w^2 ] - [ \textstyle{r^2 \over w} ] )
    * [ \ln(1 + w) ]
\Big)
/ [ 2 (1 + r)^2 (w + r)^2 ].
\end{multline*}

We used the following partition parameter values:
\begin{center}
\begin{tabular}{ l | c | c | c | r }
$R_1$   & $R_2$     & $R_3$     & $R_4$     & $R_5$ \\
\hline
$10^2$  & $10^2$    & $10^3$    & $10^1$    & $10^2$
\end{tabular}
\end{center}
