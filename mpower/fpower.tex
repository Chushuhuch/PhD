...
\end{equation*}
Where $p(x) \ge 1$ is a continuous function defined on $[-1, 1]$, $u \in \Wf[-1, 1]$, $u \ge 0$.
...
The condition of joint convexity of function $K$ is in fact certain condition on function $p$.
...
$$
{\bf H} = \frac{(1 + w)^{q - 1}}{4} \Big( w (w q + 1) (q + 1) \Ln (q'^2 \Ln + 2 q'') - q'^2 ( (1 - q w) \Ln - 2 w )^2 \Big).
$$
Note, that $K$ is always convex in $s$, and provided $p$ is convex, $K$ is convex in $x$ as well.
Then convexity of $K$ is equivalent to ${\bf H} \ge 0$.
It leads to the condition
$$
q q'' \ge q'^2 \frac{q}{q + 1} \cdot \frac{ ( (1 - q w) \Ln - 2 w )^2 - w (w q + 1) (q + 1) \Ln^2 }{ 2 w (w q + 1) \Ln }
=: q'^2 A(q, w).
$$
...
$p$ should be even and convex in $y$, and function
$$
\K_{x'}(y, d, c) = c \big({\textstyle1 + {1 + d^2 \over c^2}}
\big)^{p(x', y) \over 2}
$$
...
w|=\frac{1}{c_1}$ is truncated cone above this ellipsoid. Direct
computation gives
...
\begin{eqnarray*}
0 &\le& (I(u) - Z) - (I(u^*) - Z)
\\ &\le& \big( \K_{\bar{x}_1'}(\bar{y}_1, d_1, c_1)-1\big) |A_1| + \big(\K_{\bar{x}_2'}(\bar{y}_2, d_2, c_2)-1\big) |A_2|
+ \big( (1 + w^4)^{\frac{P}{2}} - 1 \big) |A_\delta|
\\ &&-\big( {\textstyle\K_{\hat{x}_1'}(\hat{y}_1, \frac{d_1 + d_2}{2}, \frac{c_1 + c_2}{2})-1 + \K_{\hat{x}_2'}(\hat{y}_2, \frac{d_1 + d_2}{2},
\frac{c_1 + c_2}{2})-1 }\big) |A_1'|
\\ &\le& \big( {\textstyle \K_{\bar{x}_1'}(\bar{y}_1, d_1, c_1) + \K_{\bar{x}_2'}(\bar{y}_2, d_2, c_2)} \big) C_1 w^{n + 2}
+ ({\textstyle\frac{P}{2} + o_w(1)}) C (y_2 - y_1) w^{n + 3}
\\ &&-\big( {\textstyle\K_{\hat{x}_1'}(\hat{y}_1, \frac{d_1 + d_2}{2}, \frac{c_1 + c_2}{2}) +\K_{\hat{x}_2'}(\hat{y}_2, \frac{d_1 + d_2}{2}, \frac{c_1 +
c_2}{2})} \big) C_1 w^{n + 2}.
\end{eqnarray*}
...
$$
%\begin{multline*}
\K_{x_0'}(y_1, d_1, c_1) + \K_{x_0'}(y_2, d_2, c_2) \ge
\K_{x_0'}\big({\textstyle\frac{y_1 - y_2}{2}, \frac{d_1 + d_2}{2},
\frac{c_1 + c_2}{2}}\big) + \K_{x_0'}\big({\textstyle\frac{y_2 -
y_1}{2}, \frac{d_1 + d_2}{2}, \frac{c_1 + c_2}{2}}\big),
$$
%\end{multline*}
...
$$
\textstyle
\K(y, d, c)=K(y,\frac c{\sqrt{1+d^2}})\cdot \sqrt{1+d^2},
$$
where the function $K$ is introduced in Theorem \ref{necessary}. Direct calculation gives
$$
\det(\K''(y, d, c))=\frac 1{(1+d^2)^{\frac 32}}\cdot\big[(K_{yy}K_{ss}-K_{sy}^2)(K-K_ss)-K_{ss}K_y^2d^2\big]
$$
(here $s=\frac c{\sqrt{1+d^2}}$). Therefore, if $K_y\not\equiv0$ we can choose $d$ sufficiently large so that  $\det(\K''(y, d, c))<0$, a contradiction.
\end{proof}
...
\bibitem{Shar} I.~I.~Sharapudinov, ``On certain problems of approximation theory in variable-exponent Lebesgue spaces'',
Mathematics monographs series~{\bf5},
Vladikavkaz (2012) (in Russian)
