...
\begin{lm}
\label{Wapprox}
Пусть $a$ --- непрерывная функция, $a(\cdot, u)$ четна и убывает на $[0, 1]$ для всех $u \ge 0$.
Тогда
$\forall u \in \W(-1, 1)\ \exists u_m \in Lip[-1, 1]: u_m \to u$ в $\W(-1, 1)$ и $I(u_m) \to I(u)$.
\end{lm}
...
$$I_2(v_h)=\int_{[0, 1] \setminus A_h} F( u(x), a( x, u(x) ) u'( x ) ) dx +
...
а также чтобы сделать аппроксимацию липшицевой.
...
Рассмотрим некоторую функцию $\phi_h \in \W(0, 1)$ такую, что
\begin{align*}
\phi_h( 0 ) &= 0 & & \\
...
Заметим, что $\int_0^1 \abs{\phi_h'} dx \le 2 + \sum \abs{ \beta_{h,k} } < \infty$.

{\bf Факт 1: } $\phi_h \to Id$ в $\W(0, 1)$, где $Id( x ) = x$.

Достаточно доказать, что $\phi_h' \to 1$ в $L_1(0, 1)$:
$$\int \abs{ \phi_h' - 1 } dx = \sum \Big( \frac{ \abs{ \beta_{h,k} } }{ \alpha_{h,k} } \vee 1 - 1 \Big) \alpha_{h,k} \le
\sum \abs{ \beta_{h,k} } \to 0$$.
...
( \phi_h^{-1} )' &=  \frac{ \alpha_{h,k} }{ \abs{ \beta_{h,k} } } \wedge 1 & \text{ в } & [0, 1] \cap \phi_h( \Omega_{h,k} ).
\end{align*}
...
\sign{ \beta_{h,k} } ( 1 \wedge \frac{ \abs{ \beta_{h,k} } }{ \alpha_{h,k} } ) & \text{ в } & [0, 1] \cap \phi_h( \Omega_{h,k} ).
\end{align*}

Тем самым, $u_h$ липшицева, поскольку $u$ имеет вне $A_h$ ограниченную производную.

{\bf Факт 3: } $u_h \to u$ в $\W( [0, 1] )$.

Достаточно доказать $u_h' \to u'$ в $L_1(0, 1)$:
...
Далее, $P_h^2 \le \abs{ \phi_h( A_h ) } \to 0$.
Наконец, $P_h^3 \to 0$ по абсолютной непрерывности интеграла.
...
$$M_F = \max\limits_{[-2r, 2r] \times [-M_a, M_a]} F;\ M_a = \max\limits_{[0, 1] \times [-2r, 2r]} a.$$
...
\begin{cor}
Если функция $a$ непрерывна, четна и убывает на $[0, 1]$, то для любой $u \in \W(-1, 1)$ выполнено $I( u^* ) \le I( u )$.
\end{cor}
