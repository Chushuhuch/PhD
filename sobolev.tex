...
Представим $A_h = \cup_k \Omega_{h,k}$, где $\Omega_{h,k} = ( b_{h,k}^-, b_{h,k}^+ )$. $\alpha_{h,k} = \abs{\Omega_{h,k}}$,
...
Заметим, что $\int_0^1 \abs{\phi_h'} dx \le 2 + \sum_k \abs{ \beta_{h,k} } < \infty$.
...
$$\abs{\phi_h( A_h )} = \sum \abs{ \phi_h( \Omega_{h,k} ) } = \sum \max (\abs{\beta_{h,k}}, \alpha_{h,k})
