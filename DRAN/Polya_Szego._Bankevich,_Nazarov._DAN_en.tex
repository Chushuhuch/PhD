...
For a measurable set $E \subset \Omega$, define the sets 
$$ E^* = [1 - \abs{E}, 1], \qquad 
...
function even and convex with respect to the second argument.
Let also $a: \Omega \times \mathbb R \to \bar{\mathbb R}_+$ be a continuous function. 
...
integral in (\ref{functional}) converges. 
...
form (\ref{functional}) with a weight $a$ even and convex in $x$. Thus, 
...
$u \in {\cal M}$ we succeed only under some additional restriction.

In our proof we modify the construction from \cite{ASC}. In this paper
...
both the class of functions $F$ and the class of admissible weights in this
paper are non-optimal. Our results for this case formulated in the
end of Section \ref{sobolSection} are stronger than the ones in \cite{Lan}. 

\section{Necessary conditions on the weight} 

Recall that the function $a$ is assumed nonnegative. 
...
\rem If $a$ is concave and is even in $x$, then it satisfies 
(\ref{almostConcave}). 
...
\rem Let the assumptions of the Proposition \ref{period} hold.
If, in addition
...
as the set of functions $u$, piecewise absolutely continuous on $[-1, 1]$ such that 
...
\begin{thm} 
\label{lipthm} 
Let the weight function $a$ satisfy the assumptions of the Theorem \ref{linthm}. Then for 
...
\end{equation} 
Then the result of \cite[Theorem 1, part 2]{Br}, the formula (\ref{relation}) and the Theorem
\ref{linthm} imply $u_n^*\rightharpoondown u^*$. Applying the proposition 
...
\begin{thm} 
\label{Wapprox} 
Let the weight function $a$ satisfy the assumptions of the Theorem \ref{linthm},
and let it decrease
...
monotone weight. This allows us construct a sequence of Lipschitz 
functions $u_n$, satisfying (\ref{convergence}). The result follows from 
the Theorem \ref{lipthm} and the Proposition \ref{lowersemi}. 
...
$\mathfrak{a}$ is piecewise monotone and is constant in a neighbourhoods
...
\begin{thm} 
\label{restthm} 
Let one of two conditions hold:
\begin{enumerate} 
...
Similarly to the Theorem \ref{main}, we proceed in two steps.
At first we prove (\ref{toprovesymm}) for the functions $a$, constant in $x$
in some neighborhood
of zero, and then we do for the general case.

\begin{acknowledgments}
This work was supported by RFBR grant 09-01-00729 and Federal Program 
????????.
\end{acknowledgments}
