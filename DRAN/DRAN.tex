...
Пусть $F: \mathbb R \times \mathbb R \to \mathbb R_+$ --- непрерывная
функция, четная и выпуклая по второму аргументу,
$a: \Omega \times \mathbb R \to \mathbb R_+$ --- непрерывная функция.
Введем функционал
\begin{equation}
\label{functional}
...
линейной функции $u$, вес $a$ должен быть четным по $x$, то есть
$$\forall x \in [-1, 1],\quad \forall u \in \mathbb R \quad a(x, u)=a(-x, u).$$
\end{prop}

\begin{prop}
Для того, чтобы неравенство (\ref{toprovemon}) выполнялось для любой строго
выпуклой $F$ и любой кусочно линейной функции $u$, вес $a$ должен удовлетворять
условию
\begin{equation}
\label{almostConcave}
...
$a(x_0, u_0) = 0$, то $a(1, u_0) = 0$ и функция $a(\cdot, u_0)$ периодична по
$x$ на $[x_0, 1]$.
\end{prop}

\rem Если, вдобавок к условиям предложения \ref{period}, функция $a(\cdot, u)$
четна, то она периодична на всем отрезке $[-1, 1]$ с периодом, делящим $x_0+1$
нацело. В частности, для любого неотрицательного целого $k$, такого, что
$-1 + k (x_0 + 1) \leqslant 1$ имеем $a(-1 + k (x_0 + 1), u_0) = 0$.
...
$\forall u \in \widetilde{\cal M}(a)$ справедливо неравенство
(\ref{toprovemon}).
\end{thm}
