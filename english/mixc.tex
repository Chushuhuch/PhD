...
\begin{center}
{\it Dedicated to our Teacher, Nina N. Ural'tseva, in occasion of her jubilee.}
\end{center}
...
then either $\mathfrak a \equiv 0$ on $[x_0, 1]$
...
The first part of the proof overlaps with \cite{ASC}, but we present a complete proof here for the reader's convenience.
...
Then for any $f \in \Ls(\Real)$ we have $f(\phi_h) \to f$ in $\Ls(\Real)$.
\end{prop}
...
\section{The case of symmetric rearrangement}
...
\bibitem{BB} G.~Buttazzo, M.~Belloni: A survey on old and recent results about the
gap phenomenon,
