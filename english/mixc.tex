...
However, this assertion is not justified and general speaking is not true.
...
functionals for which the Lavrentiev phenomenon takes place can be found in e.g. \cite{BGH}.
...
$$\bar{x} - \eps \le x \le \bar{x}, \bar{v} \le v \le \bar{v} + \eps \Rightarrow \mathfrak a(x, v) < \mathfrak a(-x, v).$$
...
This implies the set of roots of $\mathfrak a$ is periodic on the segment $[x_0, 1]$
...
\section{Extension of class of functions for which $I(\mathfrak a, u^*) \le I(\mathfrak a, u)$ holds}
...
\begin{equation}
\label{frDer}
\int_{L_{m,\eps}} \Big | F( u_m( x ), \mathfrak a( x, u_m( x ) ) \abs{u_m'( x )} ) - F( u( x ), \mathfrak a( x, u( x ) ) \abs{u_m'( x )} ) \Big | dx < 2 \eps.
\end{equation}
...
$$\int_{L_\eps} \Big | F( u_{m_j}( x ), \mathfrak a( x, u_{m_j}( x ) ) |u_{m_j}'( x )| ) - F( u( x ), \mathfrak a( x, u( x ) ) |u_{m_j}'( x )| ) \Big | dx < 2 \eps_j,$$
we obtain
\begin{multline*}
A = \lim I (\mathfrak a, u_{m_j}) = \lim \int_{-1}^1 F(u_{m_j}(x), \mathfrak a(x, u_{m_j}(x)) | u_{m_j }'(x) |) dx \\
\ge \varliminf \int_{-1}^1 \chi_{L_\eps}(x) F(u (x), \mathfrak a(x, u(x)) | u_{m_j}'(x) |) dx
=: \varliminf J_\eps(u_{m_j}').
\end{multline*}

The functional
$$J_\eps( v ) = \int_{-1}^1 \chi_{L_\eps}( x ) F( u( x ), \mathfrak a( x, u( x ) ) |v( x )| ) dx$$
is convex.
...
$I(\mathfrak a, u_k) \to I(\mathfrak a, u)$.
...
$$u_k \to u \text{ in } \W(-1, 1) \Rightarrow \overline{u_k} \rightharpoondown \overline{u} \text{ in } \W(-1, 1).$$
...
By the Lebesgue theorem, $u_k \to u$ in $\W(-1, 1)$, and $I(\mathfrak a, u_k) \to I(\mathfrak a, u)$.
...
\section{The inequality for $u \in \W$ with an additional restriction on weight}
\label{ASC}
...
\begin{proof}
The inequality follows from Lemma \ref{uplift} and \ref{Wapprox} immediately.
\end{proof}
