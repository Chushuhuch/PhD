...
Thereby, $\mathfrak b_{\ell}$ satisfy $(H1)-(H6)$. Therefore, $I(\mathfrak b_{\ell}, u^*) \le I(\mathfrak b_{\ell}, u)$.
Passing to the limit, we obtain (\ref{toprove}).

\bigskip
{\bf Step 4.} Let the weight $\mathfrak a$ satisfy the conditions $(H1)-(H3)$.
Then the inequality (\ref{toprove}) holds.

Suppose the function $\mathfrak a$ satisfies $(H1)-(H3)$, in particular $I(\mathfrak a, u) < \infty$.
...
and thus $\mathfrak a_k$ may not be admissible for $u$.
...
$\mathfrak c_k$ piecewise linear in $x$. Moreover
\begin{multline*}
...
It is clear that the maximum $\tau$ is zero only if $\mathfrak c(\cdot, v) \equiv 0$, otherwise the condition $(H3)$ is violated.
Function $t$ may not be continuous. However, it is easy to see that it is lower semicontinuous.
Next we take
$$\tilde{t}(v) := \inf_{w \in u([-1, 1])} \{t(w) + |v - w|\}.$$
It is obvious that $\tilde{t} \le t$, and the set of zeros of $t$ and $\tilde{t}$ coincide.

Let us show that $\tilde{t}$ is continuous (and even Lipschitz).
Fix some $v_1$.
...
is even in $x$, satisfies (\ref{almostConcave}) corresponding to Lemma \ref{maxSumConcave},
...
Moreover, it is obvious that $\mathfrak d_\alpha$ satisfies $(H1)-(H4)$.
...
Introduce the set of $\alpha$, which are "bad"\ on $[x_i, x_{i + 1}]$:
\begin{multline*}
...
\end{aligned}
$$
We have $\card(A_i) = \card(\{ \alpha \in [0, 1]: meas \{ v \in [\min u, \max u]: h(v) \pm \frac{2}{k} \alpha = 0 \} > 0 \}).$
Then $\card(A_i) \le \aleph_0$, and $\card(\cup_i A_i) \le \aleph_0$.
And thus there exists a sequence of weights $\mathfrak d_{\alpha_j} \searrow \mathfrak c$, satisfying $(H1)-(H5)$.
...
while $k \to \infty$.
Moreover, $F\big(u(x), \mathfrak a(x, u(x)) |u'(x)|\big)$ is an integrable majorant for
for the left hand integrand.
...
\bigskip
{\bf Step 5.} Let the weight $\mathfrak a$ satisfy only the condition $(H1)$.
Then the inequality (\ref{toprove}) holds.

We construct an approximation for weights $\mathfrak a$ satisfying $(H1)-(H2)$.
We apply Lemma \ref{zeroApprox} with $W = \{ v \in \Real: \mathfrak a(\cdot, v) \equiv 0 \}$.
...
We claim that $Z_{\mathfrak a(v_0)} = [-1, 1]$.
Lemma \ref{periodicity} implies that the set $Z_{\mathfrak b_{\ell}}(v_m) = Z_{\mathfrak a(v_m)}$
...
But this means that for every $v$ such that $\abs{v - v_0} \le \frac{1}{k}$,
we have $\mathfrak b_{\ell}(\cdot, v) \equiv 0$,
which contradicts $\card(Z_{\mathfrak b_{\ell}})(v_m) < \infty$.

Now we fix $k \in \Nat$ and denote $\mathfrak b_{\ell} =: \mathfrak b$.
Let us approximate the function $\mathfrak b$ with weights satisfying $(H1)-(H3)$.
It follows from $(H2)$ and Lemma \ref{periodicity}, part 2, that there exists a set $T \subset [-1, 1]$
...
We use Lemma \ref{zeroApprox} again with $W = u(T) \cup u^*(T)$.
...
Obviously, for $j$ sufficiently large $\min\limits_{dist(x, u^{-1}(v)) \le \frac{2}{j}} \mathfrak c_j(x, v)$ is not equal to zero
for $v \in U(\mathfrak c_j)$.
Moreover, for  $v \in U(\mathfrak c_j)$ we have
$$
...
$$
In this case, the denominator of the second fraction is separated from zero for $v \in U(\mathfrak c_j)$.
Thus, $D_j(\mathfrak c_j, U(\mathfrak c_j))$ is bounded.

Since $D_j$ does not change if we multiply the first argument by a factor independent of $x$,
...
Thus, the weights $\mathfrak c_j$ satisfy $(H1)-(H3)$.
Then, $I(\mathfrak c_j, u^*) \le I(\mathfrak c_j, u)$.
...
\begin{thm}
Let $F \in \mathfrak{F}$, the function $u \in \W(-1, 1)$ be nonnegative, $u(-1) = 0$,
and let the weight function $\mathfrak a: [-1, 1] \times \Real_+ \to \Real_+$ be continuous
and satisfy $(\ref{almostConcave})$.
Then the inequality $(\ref{toprove})$ holds.
\end{thm}
...
\bigskip
\noindent
$(H7')$ $(H7)$ is satisfied and $\mathfrak a(\cdot, v) \equiv 0$ in some $v$-neighborhood of zero.

\bigskip
{\bf Step 1.} Let $u \in \W(-1, 1)$, $u(-1) = 0$ and let the weight $\mathfrak a$ satisfy the conditions $(H1'), (H2)-(H6), (H7')$.
Then the inequality (\ref{toprove}) holds.

To prove this we approximate the function $u$ in the same way as in the first step of the Theorem \ref{mainThm} proof,
changing $u$ in a neighborhood of $x = -1$ to a linear function with $u_n(-1) = 0$ preserved.

\bigskip
{\bf Step 2.} Let weight $\mathfrak a$ satisfy conditions $(H1'), (H2)-(H6)$.
Then the inequality (\ref{toprove}) holds.

To prove this we add zero to the set $W$ from the second step of the Theorem \ref{mainThm} proof,
