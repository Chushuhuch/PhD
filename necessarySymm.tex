...
Возьмем $F(v, p) := p^\alpha$, где $\alpha > 1$.
Тогда
$$J = \frac{1}{\eps^\alpha} \int_0^{\eps} \big( a(s + z, \bar{v} + z)^\alpha + a(t - z, \bar{v} + z)^\alpha
...
Если взять $\alpha := \log_{ \frac{A}{\eps} }(\frac{A}{\eps} + \delta) > 1$,
то для $p \le \frac{ A }{\eps}$ имеем $p - \delta \le F( v, p ) \le p$, и
\begin{equation*}
