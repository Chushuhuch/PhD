...
В работе \todo{removed}%\cite{1dim}
восполнены пробелы в доказательстве \cite{Brock}, и результат доказан для естественного класса функций $u$.
Аналогичные результаты для монотонной перестановки также получены в \todo{removed}%\cite{DAN} и \cite{1dim}.
...
Можно проверить (\todo{removed}%\cite[лемма 10]{1dim}), что отсюда следует чётность и выпуклость функции $p$.
