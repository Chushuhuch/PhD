\section{Extension of class of functions for which inequality (\ref{toprove_weighted}) holds}
...
$$u_k \to u \text{ in } \W(-1, 1) \quad \Longrightarrow \quad \symm{u_k} \rightharpoondown \symm{u} \text{ in } \W(-1, 1).$$
Since $\mon{u_k}( x ) = \symm{u_k}( \frac{x - 1}{2} )$ and $\mon{u}( x ) = \symm{u}( \frac{x - 1}{2} )$,
...
\begin{cor}
Let the weight $\mathfrak a$ be continuous, and let inequality $(\ref{toprove_weighted})$ hold for nonnegative piecewise linear functions $u$.
