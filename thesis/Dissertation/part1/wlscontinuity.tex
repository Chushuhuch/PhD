...
\mytodo{проверить ещё раз, что всё ок в многомерном случае}

\mytodo{кое-где вместо 2 вылезет $\abs{\Omega}$}
...
Так как $u_k' \rightharpoondown u'$ в $\Ls$, по предложению \ref{prop:convex_combination_convergence} можно подобрать последовательность выпуклых комбинаций $u_k'$,
...
$w_k := \sum_{l = 1}^k \alpha_{k,l} u_{l}' \to u'$ в $\Ls$.
...
Наконец, поскольку $w_k \to u'$ в $\Ls(\overline{\Omega})$, переходя к подпоследовательности, можем считать, что $w_k(x) \to u'(x)$ почти всюду.
