...
Также заметим, что функция $L$ убывает по $s$ ($L_s(s, x) = (1 + {1 \over s^2})^{{p(x) \over 2} - 1} ( 1 + {1 \over s^2} - 2 p ) - 1$).

\begin{multline*}
\sum\limits_{k = 1}^{m} L(s_k, x_k)
\ge \sum\limits_{k = 1}^{m / 2} ( L(s_k, x_k) + L(s_{m - k + 1}, x_{m - k + 1}) ) \\
\ge \sum\limits_{k = 1}^{m / 2} 2 L({s_k + s_{m - k + 1} \over 2}, {x_{m - k + 1} - x_k \over 2})
\ge 2 L({s_1 + s_m \over 2}, {x_m - x_1 \over 2}) \\
