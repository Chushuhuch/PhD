...
на каждом из которых функция $u$ совпадает с некоторой линейной функцией $y_j^k$, $k = 1, \dots, m_j$.
Для удобства считаем, что носители $y_j^k$ для каждого $j$ идут по порядку,
то есть $\sup dom(y_j^k) \le \inf dom(y_j^{k + 1})$.
Обозначим $b_j^k = y_j^k{}'(x)$.
Тогда
\begin{multline}
I(u) = \sum\limits_j \int\limits_{u^{-1}(U_j)} (1 + u'^2(x))^{p(x) \over 2} dx
= \sum\limits_j \sum\limits_k \int\limits_{dom(y_j^k)} (1 + {y_j^k}'^2(x))^{p(x) \over 2} dx
\\ = \sum\limits_j \int\limits_{U_j} \sum\limits_k {1 \over |b_j^k|} (1 + (b_j^k)^2)^{p((y_j^k)^{-1}(y)) \over 2} dy
\end{multline}
...
Тогда для каждого $j$ можно, используя функции $y_j^k$, выразить функцию $(u^*)^{-1}$ на участке $U_j$ следующим образом
(здесь $y \in U_j$):
$$(u^*)^{-1} (y) = {1 \over 2} \sum\limits_{k = 1}^{m_j} (-1)^k (y_j^k)^{-1}(y),
\ |((u^*)^{-1})'(y)| = {1 \over |u^*{}'((u^*)^{-1}(y))|} = {1 \over 2} \sum\limits_{k = 1}^{m_j} {(-1)^k \over b_j^k} =: { 1 \over b_j^* }.$$
Тогда
\begin{multline}
I(u^*) = 2 \int\limits_0^1 (1 + u^*{}'^2(x))^{p(x)} dx
= 2 \int\limits_U {1 \over |(u^*)'(y)|} ( 1 + { 1 \over ((u^*)^{-1})'(y)^2 } ) dy
\\ = 2 \sum\limits_j \int\limits_{U_j} {1 \over |b_j^*|}
\Big(1 + b_j^*{}^2 \Big)^{p\big({1 \over 2} \sum\limits_{k = 1}^{m_j} (-1)^k (y_j^k)^{-1}(y)\big)} dy
\end{multline}
