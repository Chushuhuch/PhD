...
В работе \cite{1dim} результат \cite{Br} доказан для естественного класса функций $u$.
...
Полагая $s = t$, из неравенства $\I(u^*) \le \I(u)$ получаем, что $p$ четна и выпукла.
Далее, рассматривая произвольные $s$ и $t$, получаем выпуклость $K$.
...
Чтобы доказать неравенство для функций $u$ общего вида, нам понадобится
...
найдется последовательность функций $u_n \in C_0^\infty( -1, 1 )$, для которых
$u_n \to u$ в $\Wf[-1, 1]$ и $\I(u_n) \to \I(u)$.
\end{prop}
Легко видеть, что можно считать функции $u_n$ кусочно линейными.
...
По лемме \ref{linear} имеем $\I(u_n^*) \le \I(u_n)$.
...
Переходя к пределу при $\eps \to 0$, по теореме о монотонной сходимости получим $\I(u^*) \le \I(u)$.
\end{proof}
