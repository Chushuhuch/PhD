...
выпукла.
То есть $K(s, x_1) + K(t, x_2) \ge 2 K({ s + t \over 2 }, { x_1 + x_2 \over 2 } )$ для любых $s, t, x_1, x_2$.
\end{prop}
...
Устремив $s \to \infty$ и воспользовавшись разложением в ряд Тейлора, получим
$${ 1 \over s^2 } p(x_1) + { 1 \over s^2 } p(x_2) \ge { 1 \over s^2 } p( { x_2 - x_1 \over 2 } ) + { 1 \over s^2 } p( { x_1 - x_2 \over 2 } ) + o({ 1 \over s^2 }).$$

То есть,
$$p(x_1) + p(x_2) \ge p( { x_2 - x_1 \over 2 } ) + p( { x_1 - x_2 \over 2 } ).$$

Воспользовавшись леммой 10 из \cite{1dim}, получаем, что $p$ четна и выпукла.
Тем самым, подставив в (\ref{preConv}) $x_1$ и $-x_2$ и воспользовавшись четностью $p$, получаем
