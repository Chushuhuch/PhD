...
\begin{equation}
\label{breq}
I( \overline{u} ) \le I( u ), \qquad \qquad u \in \W(\Real^n), u \ge 0,
\end{equation}
...
Выберем для определенности $m(k)$ так, чтобы $\int_{\Omega \setminus A_{m(k)}} F( x', u( x ), \norm{\mathcal{D} u} ) dx < \frac{1}{k}$.
Тем самым первое слагаемое правой части (\ref{est}) будет стремиться к нулю.
Второе слагаемое (\ref{est}) стремится к нулю по теореме Лебега (на множестве $A_{m(k)}$ мы нашли суммируемую мажоранту).

Осталось оценить
\begin{multline*}
\int_{\Omega \setminus A_m( k )} F( x', u_k( x ), \norm{\mathcal{D} u_k} ) dx
\le \int_{\Omega \setminus A_m( k )} C_2 ( 1 + |u_k( x )|^{q^*} + |\nabla u_k( x )|^q ) dx \\
\le \int_{\Omega \setminus A_m( k )} C_2 ( 1 + |u( x )|^{q^*} + |\nabla u( x )|^q )) dx \\
+ \int_{\Omega \setminus A_m( k )} C_2 ( 1 + |u( x ) - u_k( x )|^{q^*} + |\nabla ( u( x ) - u_k( x ) )|^q ) dx.
\end{multline*}
Первое слагаемое стремится к нулю по абсолютной непрерывности интеграла.
Для второго слагаемого выполнено
\begin{multline*}
\int_{\Omega \setminus A_m( k )} C_2 ( 1 + |u( x ) - u_k( x )|^{q^*} + |\nabla ( u( x ) - u_k( x ) )|^q ) dx \\
\le C_2 ( | \Omega \setminus A_m( k ) | + \Vert u - u_k \Vert_{W^1_q}^{q^*} + \Vert u - u_k \Vert_{W^1_q}^q ) \to 0
\end{multline*}
