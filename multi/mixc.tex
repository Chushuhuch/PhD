...
и доказываем неравенство (\ref{toprove}) при некоторых ограничениях.
...
Автор \cite{Br} доказывает неравенство в случае, когда вес $a$ --- четная по $y$ функция.
Для этого неравенство (\ref{breq}) доказывается для кусочно линейных функций $u$,
после чего производится переход к пределу.
Однако, предельный переход должным образом не обоснован,
...
то как раз требуемые для перехода (d) неравенства обеспечивает предложение \ref{weightSum}.
\end{rem}
