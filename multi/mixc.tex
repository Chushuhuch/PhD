...
\author{
С.~В.~Банкевич
\footnote{Россия; Sergey.Bankevich@gmail.com}
\and
А.~И.~Назаров
...
после чего производится переход к пределу.
...
Здесь мы приводим переформулировки для многомерного случая.

Кроме того, в данном параграфе мы для простоты опускаем параметры $x'$ и $u$ у функции $a$,
подразумевая, что все тождества выполняются для любых $x' \in \omega, v \in \Real_+$.
...
В следующем предложении, как и в предыдущем параграфе, параметры $x'$ и $u$ веса $a$ опущены для простоты.
