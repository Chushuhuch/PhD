...
\begin{rem}
\label{lanLin}
Заметим, что если $u(\cdot, -1) \equiv 0$, то в переходе (d) цепочки неравенств (\ref{chain}) не нужна четность веса,
поскольку в этом случае всегда $u(x'_0, -1) < u_0$, и как раз требуемые для перехода (d) неравенства обеспечивает
предложение \ref{weightSum}.
\end{rem}
