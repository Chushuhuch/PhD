...
for $\frac{1}{q^*} = \frac{1}{q} - \frac{1}{n}$  if $q < n$, and some arbitrary $q^*$ otherwise.
...
Since $u_k$ and their derivatives are uniformly bounded
...
Indeed, $\partial \Omega \in Lip$ hence $u$ can be extended to a large ball to become a finite function.
We approximate the extension by smooth finite functions.
Next the ball is triangulated, and the function is linearly interpolated.
...
The proof of this convergence can be reduced to the Krasnoselsky's theorem on the continuity of Nemytskij operator (see. \cite[ch. 5, \textsection 17]{Kr}).
...
Else $W^1_q(\Omega)$ is embedded into $C(\overline{\Omega})$,
...
\end{eqnarray*}
almost everywhere.
Applying the Sobolev embedding theorem we get $\Vert u_k \Vert_{q^*} \le C_3 \Vert u_k \Vert_{W^1_q}$.
Thus, we found an integrable majorant, and
