...
Известно, что $\W(-1, 1)$ компактно вкладывается в $L_1(-1, 1)$. Тогда, переходя к подпоследовательности,
...
\begin{prop}
$\cite[\S6.6, \text{ Теорема } 1]{Gariepy}$
\label{smthAppr}
Пусть $f: \Real \to \Real$ липшицева.
Тогда для каждого $\eps > 0$ найдется $C^1$-функция $\bar{f}: \Real \to \Real$, такая, что
$meas\{x: \bar{f}(x) \neq f(x) \text{ или } \bar{f}'(x) \neq f'(x) \} \le \eps.$
\end{prop}
...
\end{cor}
\begin{proof}
Ввиду предложения \ref{smthAppr} липшицеву функцию можно почти всюду вместе с производной приблизить непрерывно дифференцируемыми.
